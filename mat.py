...
def matrix_vector_mul(M, v):
    ...
    :param M: R x C matrix
    :param v: C vector
    :return: R vector where r[i] = sum(M[Ri, C0-j]
...
def matrix_matrix_mul(A, B):
    ...
    :param A: Ra x Ca matrix (e.g. 3 x 5)
    :param B: Rb x Cb matrix (e.g. 5 x 4)
    :return: Ra x Cb matrix (e.g. 3 x 4)
    ...
    cols_row = {c:A * Vec(B.D[0], {r:B[r, c] for r in B.D[0]}) for c in B.D[1]}

    return Mat((A.D[0],B.D[1]), {(d, c):rv[d] for c, rv in cols_row.items() for d in rv.D})
